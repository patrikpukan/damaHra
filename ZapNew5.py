...
def noPiecesBetween(board, old_x, old_y, new_x, new_y):
  ...
  # Allows queen players to move one spot over (like a pawn would move)
    ...
    print("You can't jump there. Please try another move.")
    return False
...
def checkIfDoubleJumpPossibleQueen():
  # Checking for queen double jump possibility
  global moznost, board,new_X,new_y
  if board[new_y][new_x] == enemy['queen']:
    if moznost == True:
      moznost=False
    try:
      for i in range(8):
        # North East
        if board[new_y - i][new_x + i] == friendly['pawn'] or board[new_y - i][new_x + i] == friendly['queen']:
          if board[new_y - i-1][new_x + i+1] == empty:
            moznost = True
            print(f"moznostNE {i} {new_y - i-1} {new_x + i+1}")
            return True
        # North West
        if board[new_y - i][new_x - i] == friendly['pawn'] or board[new_y - i][new_x + i] == friendly['queen']:
          if board[new_y - i-1][new_x - i-1] == empty:
            moznost = True
            print(f"moznostNW {i} {new_y - i-1} {new_x + i-1}")
            return True
        # South East
        if board[new_y + i][new_x + i] ==  friendly['pawn'] or board[new_y - i][new_x + i] == friendly['queen']:
          if board[new_y + i+1][new_x + i+1] == empty:
            print(f"moznostSE {i} {new_y - i+1} {new_x + i+1}")
            moznost = True
            return True
        # South West
        if board[new_y + i][new_x - i] == friendly['pawn'] or board[new_y - i][new_x + i] == friendly['queen']:
          if board[new_y + i+1][new_x - i-1] == empty:
            moznost = True
            print(f"moznostSW {i} {new_y - i+1} {new_x + i-1}")
            return True
    except IndexError:
      pass
  else:
...
def checkIfDoubleJumpPossible(board, new_x, new_y):
  ...
  if currentPlayer == 1:
    if board[new_y][new_x] == friendly['pawn']:
      if new_x == 0 or new_x == 1:
        return False
      try:
        # North East
        if board[new_y - 2][new_x + 2] == empty:
          if board[new_y - 1][new_x + 1] == enemy['pawn'] or board[new_y - 1][new_x + 1] == enemy['queen']:
  ...
  if currentPlayer == 2:
    ...
    if board[new_y][new_x] == friendly['pawn']:
      try:
          ...
          if board[new_y + 2][new_x - 2] == empty:
            if board[new_y + 1][new_x - 1] == enemy['pawn'] or board[new_y - 1][new_x + 1] == enemy['queen']:
              # if board[new_y + 3][new_x - 3] == empty or enemy['pawn'] or enemy['queen'] or friendly['pawn'] or friendly['queen']:
                print(f"new_y: {new_y} new_x: {new_x}")
                return True
      except IndexError:
        return False
            ...
            if board[old_y][old_x] == friendly['pawn']:
              if isValidMove(currentPlayer, board, old_x, old_y, new_x, new_y) is True:
                ...
                if previous_piece_total > current_piece_total:
                        # print(previous_piece_total)
                        # print(current_piece_total)
            ...
            if board[old_y][old_x] == (friendly['queen']):
              if isValidQueenMove(currentPlayer, board, old_x, old_y, new_x, new_y) is True:
                ...
                if previous_piece_total > current_piece_total:
                  ...
                  #   checkIfDoubleJumpPossibleQueen()
                  #   print(f"moznost: {moznost}")
                  # if checkIfDoubleJumpPossibleQueen(board, new_x, new_y) is True:
                  if moznost is True:
